/*
 * GOTO5MASTER.ino Written by Igor Ovchinnikov 21/07/2016
 */
...
boolean bForceIR = true;   //Ускоренный режим управления с пульта ИК
...
#include "GOTO6MASTER.h" // Этот файл должен быть в той же директории, что и GOTO6MASTER.ino
...
//     1 - замедленное управление монтировкой
//     2 - ускоренное управление монтировкой
...
//     5 - Focus-
//     6 - Focus+
//     7 - Телескоп слева от полярной оси
//     8 - Телескоп справа от полярной оси
//    10 - Сброс выравнивания монтировки
// 65536 - включить/отключить трекинг
// Используются суммы значений от 256

  do 
   {
    iMovement=0;
    iKey=AskControl();
    if (iKey ==  1) bForceIR = false; //Замедленный режим управления с пульта ИК
    if (iKey ==  2) bForceIR = true;  //Ускоренный режим управления с пульта ИК
    if (iKey ==  5) {Stepper_Z_step(-iStDZ);} // Focus-
    if (iKey ==  6) {Stepper_Z_step( iStDZ);} // Focus+
    if (iKey ==  7) {iStDY = -1;} // Телескоп слева от полярной оси
    if (iKey ==  8) {iStDY =  1;} // Телескоп справа от полярной оси
    if (iKey == 10) {bAlignment=false;} // Сброс выравнивания монтировки
