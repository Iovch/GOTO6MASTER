/*
 * GOTO5MASTER.h Written by Igor Ovchinnikov 21/07/2016
 */
...
long AskSlaveI2C()
{
 ...
 switch (iRetValue)
 {
  case 15: {iRetValue=65536; break;} // Включить/отключить трекинг
//  case 18: {iRetValue=    3; break;} // Включить трекинг
//  case 15: {iRetValue=    4; break;} // Отключить трекинг
  case  1: {iRetValue=    5; break;} // Fokus-
  case  2: {iRetValue=    2; break;} // Ускоренное управление монтировкой
  case  3: {iRetValue=    6; break;} // Fokus+
  case  5: {iRetValue=    1; break;} // Замедленное управление монтировкой
  ...
  case 11: {iRetValue= 7; break;}    // Телескоп слева от полярной оси
  case 10: {iRetValue=10; break;}    // Сброс выравнивания монтировки
  case 12: {iRetValue= 8; break;}    // Телескоп справа от полярной оси
  default: iRetValue=0;
 }
 return iRetValue; 
}
...
void HexDeToString(unsigned long ulDeVal, unsigned long ulMaxDeVal)
{
  ...
  switch (iStDY)
  {
  case -1: {LCDString2=LCDString2+" NW "; break;}
  case  1: {LCDString2=LCDString2+" NE "; break;}
  }
}
...
// 32768 - когда надо сделать полныйшаг вниз по оси У
// 65536 - включить/отключить трекинг
// Используются суммы значений более 256
...
void LCDCOR (int pKey)
{
 if(true)
 {
  ...
  if(pKey==5) LCDPrintSTR (" -v- FOCUS -vv- ", 2, 1);
  if(pKey==6) LCDPrintSTR (" +^+ FOCUS +^^+ ", 2, 1);
  switch (pKey)
  {
